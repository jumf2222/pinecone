...
export class DegreePlannerComponent implements OnInit {
  ...
  constructor(public degreeService: DegreeService, private router: Router) { }
  ...
  selectSchedule(schedule: Schedule) {
    this.router.navigate(['schedule'], { queryParams: { schedule: schedule.name, level: this.degreeService.getDegree().years.find(a => Object.values(a.schedules).includes(schedule)).level } });
  }
